...
public class CursoServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        out.print("<h3> Olá Mundo Servlet</h3>");
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        //criar objeto da classe cursoDAO
        Curso curso = new Curso();
        CursoDAO cursoDAO = new CursoDAO();

        curso.setNome(request.getParameter("txNome"));
        curso.setCargaHoraria(Integer.parseInt(request.getParameter("txChoraria")));
        
        if (request.getParameter("txId")== null || request.getParameter("txId").equals("")) {
            /*Aqui chama a função gravar do cursoDAO*/       
            cursoDAO.gravar(curso);
        } else {
            curso.setId(Integer.parseInt(request.getParameter("txId")));
            cursoDAO.editar(curso);
        }

        response.sendRedirect("curso/listar.jsp");
